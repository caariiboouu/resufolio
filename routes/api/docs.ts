    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.log(`Contents of parent directory: ${parentEntries.join(", ")}`);
        } catch (parentErr) {
        ...
        return new Response(JSON.stringify({ 
          ...
          path: docsPath,
          exists: false,
          details: statErr instanceof Error ? statErr.message : String(statErr)
        }), {
          status: 404,
          headers: { "Content-Type": "application/json" },
        });
      ...
      try {
        ...
        for (const entry of entries) {
          console.log(`Entry: ${entry.name} (${entry.isFile ? "File" : "Directory"})`);
        }
      } catch (readErr) {
        ...
        if (dirEntry.isFile && dirEntry.name.endsWith(".md")) {
          try {
            ...
            try {
              ...
              const { attrs, body } = extract(fileContent) as { attrs: FrontMatterAttrs; body: string };
              
              // If front matter extraction worked, use it
              title = attrs.title || extractTitleFromMarkdown(body) || dirEntry.name.replace(".md", "");
              content = body;
              
              console.log(`Front matter extraction successful for: ${dirEntry.name}`);
            } catch (frontMatterErr) {
              // If front matter extraction failed, fall back to just extracting title from content
              console.log(`No front matter found in ${dirEntry.name}, extracting title from content directly`);
              title = extractTitleFromMarkdown(fileContent) || dirEntry.name.replace(".md", "");
              content = fileContent;
            }
            ...
            console.log(`Successfully processed: ${dirEntry.name} with title: ${title}`);
          } catch (fileErr) {
...
Markdown files: ${entries.filter(e => e.isFile && e.name.endsWith(".md")).length}
