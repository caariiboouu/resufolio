...
export default function ResumeViewer({ pdfUrl, isOpen, onClose }: { 
  ...
  const enhancedPdfUrl = `${pdfUrl}#page=2&toolbar=0&navpanes=0&scrollbar=0&view=FitH&textlayer=0&messages=0&disablehistory=true&disabletextselection=true`;
  ...
  return (
    <>
      ...
      <div
        ...
        className={`transition-opacity duration-500 ${backdropVisible ? 'opacity-100' : 'opacity-0'}`}
      ...
      >
        ...
        <div 
          className={`bg-white transition-transform duration-500 ease-in-out ${mounted && !isClosing ? 'translate-y-0' : 'translate-y-full'}`}
          ...
          <div style={modalStyles.contentArea}>
            <div style={modalStyles.pdfContainer}>
              ...
              {!pdfFullyLoaded && (
                ...
                >
                  <div className="text-center">
                    ...
                    <p className="text-gray-600">{pdfLoading ? "Loading PDF..." : "Navigating..."}</p>
                  </div>
                </div>
              )}
              ...
              <iframe
                ref={iframeRef}
                src={enhancedPdfUrl}
                style={pdfFullyLoaded ? modalStyles.iframe : modalStyles.hiddenIframe}
                frameBorder="0"
                onLoad={handleIframeLoad}
                title="Resume PDF"
                className={`transition-opacity duration-300 ${pdfFullyLoaded ? 'opacity-100' : 'opacity-0'}`}
              >
