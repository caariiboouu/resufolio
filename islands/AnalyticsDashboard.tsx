...
export default function AnalyticsDashboard(
  { stats }: { stats: AnalyticsPageData["stats"] },
) {
  const [activeTab, setActiveTab] = useState("overview");
  const [pageViews, setPageViews] = useState<Array<Record<string, unknown>>>(
    [],
  );
  ...
  const fetchData = async (type: string) => {
    ...
    try {
      ...
      if (type === "pageviews") {
        ...
        data.forEach((view: Record<string, unknown>) => {
        ...
        data.forEach((view: Record<string, unknown>) => {
          ...
          statsByIp[ip].lastVisit = Math.max(
            statsByIp[ip].lastVisit,
            view.timestamp,
          );
          statsByIp[ip].firstVisit = Math.min(
            statsByIp[ip].firstVisit,
            view.timestamp,
          );
  ...
  useEffect(() => {
    if (
      activeTab === "overview" || activeTab === "pageviews" ||
      activeTab === "referrers" || activeTab === "visitors"
    ) {
      fetchData("pageviews");
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-3xl font-bold mb-6 text-[#32564f]">
          Analytics Dashboard
        </h1>
            ...
            <div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-[#E7DECA] p-4 rounded-lg">
                  <h3 className="text-lg font-semibold text-[#32564f] mb-2">
                    Last 24 Hours
                  </h3>
              ...
              <div>
                <h3 className="text-xl font-semibold mb-4 text-[#32564f]">
                  Top Pages
                </h3>
              ...
              {loading
                ...
                ? (
                  <div className="overflow-x-auto">
                    <table className="min-w-full bg-white">
                      ...
                      <tbody>
                        {pageViews.map((view: Record<string, unknown>) => (
                ...
                : (
                  <p className="text-gray-500">
                    No page views data available yet.
                  </p>
                )}
            </div>
          )}
                  ...
                  <div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                      <div className="bg-white p-6 rounded-lg shadow">
                        <h4 className="text-lg font-semibold mb-4 text-[#32564f]">
                          Top Referrers
                        </h4>
                        <table className="min-w-full">
                          ...
                          <tbody>
                            {referrerStats.slice(0, 10).map(
                              (referrer, index) => {
                                const totalVisits = referrerStats.reduce(
                                  (sum, item) => sum + item.count,
                                  0,
                                );
                                const percentage =
                                  ((referrer.count / totalVisits) * 100)
                                    .toFixed(1);
                      ...
                      <div className="bg-white p-6 rounded-lg shadow">
                        ...
                        <div className="h-64 flex items-end">
                          {referrerStats.slice(0, 5).map((referrer, index) => {
                            ...
                            const percentage = (referrer.count / totalVisits) *
                              100;
          ...
          {activeTab === "actions" && (
            <div>
              ...
              {loading
                ...
                ? (
                  <div className="overflow-x-auto">
                    <table className="min-w-full bg-white">
                      ...
                      <tbody>
                        {userActions.map((action: Record<string, unknown>) => (
                ...
                : (
                  <p className="text-gray-500">
                    No user actions data available yet.
                  </p>
                )}
            </div>
          )}
                      ...
                      <tbody>
                        {visitorStats.map((visitor) => (
                          <tr key={visitor.ip} className="hover:bg-gray-50">
                            <td className="py-2 px-3 border-b font-mono">
                              {visitor.ip}
                            </td>
                            ...
                            <td className="py-2 px-3 border-b text-xs">
                              {visitor.referrers.map((ref) => (
                                <div key={ref}>{ref}</div>
                              ))}
                            </td>
                            <td className="py-2 px-3 border-b text-xs">
                              {visitor.userAgents.map((ua) => (
                                <div key={ua} title={ua}>
                                  {ua.substring(0, 30)}...
                                </div>
                              ))}
